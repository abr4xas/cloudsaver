...
export class ResourceFetcher {
	...
	async fetchAllResources(token: string): Promise<AllResources> {
		...
		const _timeout = this.apiClient.getTimeout();
		...
		try {
			...
			const results: AllResources = {
				droplets: this.processResponse(dropletsRes, 'droplets') as AllResources['droplets'],
				volumes: this.processResponse(volumesRes, 'volumes') as AllResources['volumes'],
				snapshots: this.processResponse(snapshotsRes, 'snapshots') as AllResources['snapshots'],
				databases: this.processResponse(databasesRes, 'databases') as AllResources['databases'],
				reserved_ips: this.processResponse(
					reservedIpsRes,
					'reserved_ips'
				) as AllResources['reserved_ips'],
				load_balancers: this.processResponse(
					loadBalancersRes,
					'load_balancers'
				) as AllResources['load_balancers'],
			};
