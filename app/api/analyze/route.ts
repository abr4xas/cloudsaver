...
import { RecommendationEngine as AnalyzersEngine } from "@/lib/recommendations";
...
function validateToken(token: string): void {
    ...
    if (token.length < 32) {
        throw new ValidationError("Invalid token format: token too short");
    }

    // Check for basic format (alphanumeric, dashes, underscores)
    if (!/^[a-zA-Z0-9_-]+$/.test(token)) {
        throw new ValidationError("Invalid token format: contains invalid characters");
    }
}
...
async function analyzeAccount(token: string) {
    ...
    // Register analyzers from lib/recommendations
    const analyzersEngine = new AnalyzersEngine();
 ...
 */
async function handleAnalyze(request: NextRequest): Promise<Response> {
    try {
