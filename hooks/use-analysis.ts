...
import { isValidTokenFormat, getTokenValidationError } from "@/lib/validation/token-validator";
...
export function useAnalysis({ onAnalysisComplete }: UseAnalysisOptions = {}) {
	...
	const analyze = useCallback(
		async (
			...
			if (!isValidTokenFormat(tokenToAnalyze)) {
				triggerShake();
				setState("error");
				const errorMessage = getTokenValidationError(tokenToAnalyze) || "Invalid token format. Please check your token and try again.";
				setResultsData({
					monthlyCost: 0,
					potentialSavings: 0,
					savingsPercentage: 0,
